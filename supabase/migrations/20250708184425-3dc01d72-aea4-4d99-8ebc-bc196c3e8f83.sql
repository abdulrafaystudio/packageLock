...
-- Definitive fix for complete_paid_signup function conflict
-- Drop ALL possible function signatures to ensure clean state
...
DROP FUNCTION IF EXISTS public.complete_paid_signup(text, text, text);
DROP FUNCTION IF EXISTS public.complete_paid_signup(text, text);
DROP FUNCTION IF EXISTS public.complete_paid_signup(text);

-- Create the definitive version with exactly 6 parameters
...
BEGIN
    ...
    RETURN jsonb_build_object(
        ...
        'function_version', 'definitive_v1'
    );
...
'Definitive version of complete_paid_signup function with 6 parameters. Processes successful Stripe payments and prepares account data for auth user creation.';
