-- Fix enhancedsafesavestripecustomerid to use email matching from subscribers table only
...
BEGIN
    -- Get user_id if not provided - CHANGED TO USE SUBSCRIBERS TABLE ONLY
    IF p_user_id IS NULL THEN
        SELECT user_id INTO final_user_id
        FROM public.subscribers
        WHERE email = lower(trim(p_email))
        LIMIT 1;
        
        IF final_user_id IS NULL THEN
            error_message := 'No user found in subscribers table for email: ' || p_email;
